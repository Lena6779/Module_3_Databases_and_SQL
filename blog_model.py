...
# unique=True — No two rows can have the same value (maps to SQL UNIQUE)#__repr__ — Not required by SQLAlchemy, but makes debugging much easier
...
# The key insight: you're working with Python objects, not SQL strings. alice.bio is just attribute access. filter_by(published=True) reads like English. SQLAlchemy handles the translation to SQL.